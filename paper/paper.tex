...
\section{Firing Squad Synchronisation Problem}
    ...
    Imagine a group of soldiers all part of a firing squad, with a general at
    one end. Firing squads would aim to have all of its soldiers fire at the
    same time so that no one soldier could be sure he had delivered the fatal
    wound to the condemned. The soldiers of the firing squad can be modelled as
    cells in a cellular automaton, and so the Firing Squad Synchronisation
    Problem, or FSSP, is concerned with ensuring that all states end up in a
    ``firing'' state at some point, at the same point in time. At each time
    step, a soldier may change what he is doing based on what him and his two
    immediate neighbours are doing (so the two soldiers on the ends only take
    into account their own current action and that of his single neighbour).
    Assume all soldiers start off in the same inactive state, also called the
    quiescent state. The question begins when the general initially gives the
    command to fire: assuming there is a fixed number of states, regardless of
    how many soldiers are in the line, how can the states be defined such that
    all soldiers are in the firing state at the same time?
